...
class hexa_walking_control:
    ...
    def walker(self,imu_subscriber,tip1_subscriber,tip2_subscriber,tip3_subscriber,tip4_subscriber,tip5_subscriber,tip6_subscriber):
        self.x = imu_subscriber.orientation.x
        self.y = imu_subscriber.orientation.y
        self.z = imu_subscriber.orientation.z
        self.w = imu_subscriber.orientation.w
        (self.r,self.p,self.y) = euler_from_quaternion((self.x,self.y,self.z,self.w))
        #print("Roll is:"+str(math.degrees(self.r)))
        #print("Pitch is:"+str(math.degrees(self.p)))
        ...
        #print(self.x,self.y)
        if   0 <= abs(self.r) <= 0.05 and 0 <= abs(self.p) <= 0.05:
            if self.retraction2 == True:
               # print("fast"+str(self.p))
                for i in range(0,25):
